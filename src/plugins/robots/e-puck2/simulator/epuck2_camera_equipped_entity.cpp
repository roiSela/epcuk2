/**
 * @file <argos3/plugins/robots/e-puck2/simulator/epuck2_camera_equipped_entity.cpp>
 *
...
namespace argos {
   ...
   CEPuck2CameraEquippedEntity::CEPuck2CameraEquippedEntity(CComposableEntity* pc_parent) :
      CEntity(pc_parent),
      m_fFocalLength(0.0f),
      m_fRange(0.0f),
      m_nImagePxWidth(0),
   ...
   void CEPuck2CameraEquippedEntity::Init(TConfigurationNode& t_tree) {
      try {
         CEntity::Init(t_tree);
   ...
   void CEPuck2CameraEquippedEntity::Enable() {
      if (m_psAnchor) {
         m_psAnchor->Enable();
      }
   }
   ...
   void CEPuck2CameraEquippedEntity::Disable() {
      if (m_psAnchor) {
         m_psAnchor->Disable();
      }
   }
   ...
   REGISTER_STANDARD_SPACE_OPERATIONS_ON_ENTITY(CEPuck2CameraEquippedEntity);
